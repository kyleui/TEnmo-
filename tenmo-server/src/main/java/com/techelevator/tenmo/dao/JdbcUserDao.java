...
public class JdbcUserDao implements UserDao {
    ...
    public String findUsernameById(int userId) {
        if (userId == 0) throw new IllegalArgumentException("Username cannot be null");

        String username;
        try {
            username = jdbcTemplate.queryForObject("SELECT username FROM tenmo_user WHERE user_id = ?", String.class, userId);
        } catch (NullPointerException | EmptyResultDataAccessException e) {
